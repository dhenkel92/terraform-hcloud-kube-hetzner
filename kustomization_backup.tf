data "remote_file" "kustomization_backup" {
  conn {
    host        = var.is_vpn ? module.control_planes[keys(module.control_planes)[0]].private_ipv4_address : module.control_planes[keys(module.control_planes)[0]].ipv4_address
    port        = var.ssh_port
    user        = "root"
